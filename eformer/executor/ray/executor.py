...
class RayExecutor:
	...
	def execute(
		...
		**kwargs,
		...
		if accelerator_config.head_name is None and not isinstance(
			...
			resources = {
				accelerator_config.head_name or default_name: accelerator_config.head_workers
			}
	...
	def execute_multislice(
		...
		**kwargs,
		...
		actors = [
			MultisliceActor.remote(accelerator_config)
			for _ in range(getattr(accelerator_config, "pod_count", 1))
		]
		...
		try:
			...
			for actor in actors:
				try:
					...
					logger.exception(
						"Failed to kill actor after primary failure in get_slice_info"
					)
			raise e
		...
		if (
			...
			raise RayError(
				f"Could not determine coordinator IP from malformed or empty slice_infos: {slice_infos}"
			)
		...
		while (
			...
			try:
				...
				if "preempted" in str(e).lower():
					...
					num_failures += 1
					logger.warning(
						f"A slice failed (RayTaskError). Failure count: {num_failures}.", exc_info=e
					)
				continue
			except RayError as e:
				problem = e
			...
			if not all_slice_job_statuses:
				...
				num_failures += 1
				problem = problem or RuntimeError(
					"Empty job statuses from execute_multislice after ray.get()"
				)
				continue
			...
			if (
				...
				num_failures += 1
				problem = problem or RuntimeError(
					"Inconsistent state in multislice resumable after processing results"
				)
				continue

		if num_preemptions >= max_retries_preemption:
			logger.error(
				f"Multislice job preempted too many times ({num_preemptions} >= {max_retries_preemption})."
			)
			raise RuntimeError(f"Preempted too many times ({num_preemptions})") from problem
		elif num_failures >= max_retries_failure:
			logger.error(
				f"Multislice job failed too many times ({num_failures} >= {max_retries_failure})."
			)
